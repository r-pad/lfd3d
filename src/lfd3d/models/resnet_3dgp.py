...
class Resnet3DGPNetwork(nn.Module):
    """
    Resnet + 3D positional encoding + Transformer for 3D goal prediction
    Architecture:
    - Image tokens: Resnet patches with 3D PE (x,y,z from depth)
    ...
    def __init__(self, model_cfg):
        super(Resnet3DGPNetwork, self).__init__()

        # Set up backbone.
        backbone_model = getattr(torchvision.models, model_cfg.vision_backbone)(
            weights=model_cfg.pretrained_backbone_weights
        )
        # Note: This assumes that the layer4 feature map is children()[-3], and remove layer4, 
        # TODO(alexander-soare): Use a safer alternative.
        backbone_list = [nn.Sequential(*(list(backbone_model.children())[:-3])) for _ in range(model_cfg.num_cam)] # output size: 14 * 14 * hidden_dim
        self.backbone_list = nn.ModuleList(backbone_list)
        device = torch.device("cuda") if torch.cuda.is_available() else "cpu"
        self.backbone_list.to(device)
        ...
        with torch.no_grad():
            ...
            dummy_output = self.backbone_list[0](dummy_input)
            self.resnet_hidden_size = dummy_output.shape[1]

        self.hidden_dim = self.resnet_hidden_size + self.pos_encoding_dim
    ...
    def forward(
        ...
        assert C == 3 and H == 224 and W == 224, f"Expected image shape (B, N, 3, 224, 224), got {image.shape}"
        ...
        for cam_idx in range(N):
            with torch.no_grad():
                ...
                cam_image = TF.convert_image_dtype(cam_image, torch.float32)  # uint8 -> float in [0,1]
                cam_image = TF.normalize(cam_image,
                                        mean=[0.485, 0.456, 0.406],
                                        std=[0.229, 0.224, 0.225])
                resnet_outputs = self.backbone_list[cam_idx](cam_image) # (B, resnet_hidden_size, 14, 14)
            all_patch_features.append(torch.flatten(resnet_outputs, start_dim=2).permute(0, 2, 1))  # (B, 196, resnet_hidden_size)
...
class Dino3DGPGoalRegressionModule(pl.LightningModule):
    ...
    def collect_and_stack_predictions(self, batch, n_samples):
        ...
        Returns:
            ...
            weighted_displacement: Weighted displacement from first prediction
            z: Mean-pooled token representation
        """
        all_pred_dict = []
        if self.is_gmm:
        ...
        return pred_dict, weighted_displacement, z

    def calculate_pixel_metrics(self, pred_dict, batch, init, gt):
        """
        ...
        Args:
            ...
            batch: Batch data
            init: Initial gripper positions (B, 4, 3)
            gt: Ground truth gripper positions (B, 4, 3)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if do_additional_logging:
            ...
            self.eval()
            with torch.no_grad():
                pred_dict, weighted_displacement, _ = (
                    self.collect_and_stack_predictions(batch, n_samples_wta)
                )
            self.train()
    ...
    def log_viz_to_wandb(self, batch, pred_dict, weighted_displacement, tag):
        ...
        pcd_cam = self.transform_points_homogeneous(pcd, T_cam_from_world)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        self.eval()
        with torch.no_grad():
            pred_dict, weighted_displacement, _ = self.collect_and_stack_predictions(
                batch, n_samples_wta
            )
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        pred_dict, weighted_displacement, z = self.collect_and_stack_predictions(
            batch, n_samples_wta
        )
